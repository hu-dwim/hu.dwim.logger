...
(def (generic e) format-message (logger appender level stream message-control message-arguments)
  ...
  (:method :around (logger appender level (output-stream (eql nil)) message-control message-arguments)
    ;; mimic the semantics of CL:FORMAT when called on NIL output stream
    (with-output-to-string (stream)
      (format-message logger appender level stream message-control message-arguments))))
